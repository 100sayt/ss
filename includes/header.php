...
<header class="sticky top-0 z-50 glass-header w-full shadow-sm bg-white">
            ...
            <div class="flex items-center justify-between h-20 gap-8">
                    ...
                    <div class="flex-1 max-w-2xl">
                        <form @submit.prevent="submitSearch()" class="relative flex items-center gap-2 bg-slate-100 p-1 rounded-2xl border-2 border-transparent focus-within:bg-white focus-within:border-primary/20 transition-all">
                            <div class="flex items-center h-10 px-4 shrink-0 border-r border-slate-200">
                                <span class="material-symbols-outlined text-slate-400 text-[26px]">search</span>
                            </div>
                ...
                <nav class="flex items-center gap-6 shrink-0">
                    <div class="relative" @click.outside="langOpen = false">
                        <button @click="langOpen = !langOpen" class="flex items-center gap-1 text-[13px] font-black uppercase text-slate-600 hover:text-primary transition-all px-2 py-1 rounded-lg hover:bg-slate-50">
                            <?= strtoupper($current_lang ?? 'az') ?>
                            <span class="material-symbols-outlined text-[20px]" :class="langOpen ? 'rotate-180' : ''">expand_more</span>
                        </button>
                        <div x-show="langOpen" x-transition class="absolute top-full right-0 mt-2 bg-white rounded-xl shadow-2xl border border-slate-50 py-2 w-32 z-[10000]" x-cloak>
                            <button onclick="setGoogTrans('az')" class="block w-full text-left px-4 py-2.5 text-[12px] font-bold uppercase hover:bg-slate-50 hover:text-primary">AZ - Azeri</button>
                            <button onclick="setGoogTrans('ru')" class="block w-full text-left px-4 py-2.5 text-[12px] font-bold uppercase hover:bg-slate-50 hover:text-primary">RU - Ru</button>
                            <button onclick="setGoogTrans('en')" class="block w-full text-left px-4 py-2.5 text-[12px] font-bold uppercase hover:bg-slate-50 hover:text-primary">EN - En</button>
                        </div>
                    </div>
    ...
    <div class="lg:hidden w-full flex flex-col bg-white border-b border-slate-50">
        <div class="flex items-center justify-between px-4 py-3">
            ...
            <div class="flex items-center gap-2" onclick="window.location.href='/index.php'"><img src="/assets/img/logo.png" alt="" class="h-8 w-auto"><span class="text-xl font-black text-primary">RentAl</span></div>
        ...
        <div class="px-4 pb-3">
            <form @submit.prevent="submitSearch()" class="relative flex items-center bg-slate-100 rounded-xl h-12 px-3 border-2 border-transparent focus-within:bg-white focus-within:border-primary/20 transition-all">
                <span class="material-symbols-outlined text-slate-400 text-[26px]">search</span>
                <input x-model="searchQuery" class="bg-transparent border-0 outline-none text-slate-800 placeholder-slate-400 w-full text-base font-medium focus:ring-0 pl-3" placeholder="Əşya və ya xidmət axtarışı" type="text" autocomplete="off"/>
        ...
        <div x-data="{ m_langOpen: false }" class="notranslate mt-4">
            ...
            <div class="relative">
                ...
                <div x-show="m_langOpen" x-transition class="mt-2 bg-white rounded-2xl border border-slate-100 overflow-hidden shadow-xl">
                    <button onclick="setGoogTrans('az')" class="block w-full text-left p-4 font-bold hover:bg-slate-50 border-b border-slate-50 last:border-0">Azeri - AZ</button>
                    <button onclick="setGoogTrans('ru')" class="block w-full text-left p-4 font-bold hover:bg-slate-50 border-b border-slate-50 last:border-0">Russian - RU</button>
                    <button onclick="setGoogTrans('en')" class="block w-full text-left p-4 font-bold hover:bg-slate-50 last:border-0">English - EN</button>
                </div>
            </div>
        </div>
        <div class="space-y-2">
            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-4">Menyu</p>
            <a href="/index.php" class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 text-slate-900 font-bold hover:bg-primary/5 hover:text-primary transition-all"><span class="material-symbols-outlined">home</span> Əsas səhifə</a>
            <a href="/categories.php" class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 text-slate-900 font-bold hover:bg-primary/5 hover:text-primary transition-all"><span class="material-symbols-outlined">grid_view</span> Kateqoriyalar</a>
            <?php if (is_admin()): ?><a href="/admin/index.php" class="flex items-center gap-4 p-4 rounded-2xl bg-red-50 text-red-600 font-bold hover:bg-red-100 transition-all"><span class="material-symbols-outlined">admin_panel_settings</span> Admin Panel</a><?php endif; ?>
        </div>
        <div class="space-y-2 pt-4 border-t border-slate-100">
            <p class="text-[11px] font-bold text-slate-400 uppercase tracking-widest mb-4">Hesab</p>
            <?php if (is_logged_in()): ?>
                <a href="/profile.php" class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 text-slate-900 font-bold"><span class="material-symbols-outlined">account_circle</span> Profilim</a>
                <a href="/profile.php?tab=my_listings" class="flex items-center gap-4 p-4 rounded-2xl bg-slate-50 text-slate-900 font-bold"><span class="material-symbols-outlined">list_alt</span> Mənim elanlarım</a>
                <a href="/logout.php" class="flex items-center gap-4 p-4 rounded-2xl bg-red-50 text-red-600 font-bold"><span class="material-symbols-outlined">logout</span> Çıxış</a>
            <?php else: ?>
                <a href="/login.php" class="flex items-center gap-4 p-4 rounded-2xl bg-primary text-white font-bold shadow-lg shadow-primary/20"><span class="material-symbols-outlined">login</span> Giriş Edin</a>
                <a href="/register.php" class="flex items-center gap-4 p-4 rounded-2xl bg-white border border-slate-200 text-slate-900 font-bold"><span class="material-symbols-outlined">person_add</span> Qeydiyyat</a>
            <?php endif; ?>
        </div>
    </div>
</div>

<nav class="lg:hidden fixed bottom-0 left-0 w-full z-[100] mobile-nav h-16 flex items-center justify-between px-2 max-w-md mx-auto relative select-none">
        ...
        <?php $allowed_pages = ['index.php', 'listing.php']; $current_page = basename($_SERVER['SCRIPT_NAME']); if (in_array($current_page, $allowed_pages) && isset($pdo)):
            ...
            <div x-data="{ loaded: false }" x-init="setTimeout(() => loaded = true, 150)" x-show="loaded" x-transition:enter="transition ease-out duration-500" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" class="relative w-full max-w-[1000px] group leading-[0]">
                <div class="relative overflow-hidden rounded-2xl bg-white border border-gray-200 shadow-sm hover:shadow-md transition-all duration-300">
                    <div class="flex flex-col"><div class="w-full overflow-hidden flex justify-center bg-gray-50/50"><?= $ad_content ?></div><div class="bg-gray-50/50 py-0.5 px-4 flex justify-end items-center border-t border-gray-100/50"></div></div>
                </div>
