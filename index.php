...
$premium_sql = "
    SELECT l.*, c.".lang_col('name')." as category_name, cit.".lang_col('name')." as city_name, cur.symbol,
           ...
           l.rent_period, l.is_sale_possible
    FROM listings l
...
// Add common filters to premium too
    ...
    <template x-if="slides.length > 1">
        <div>
            ...
            <div class="relative z-30 flex flex-col items-center max-w-3xl w-full">
                ...
                <form action="/index.php" method="GET" class="w-full max-w-2xl bg-white p-2 rounded-2xl flex flex-col sm:flex-row items-center gap-2 shadow-2xl">

                    <div class="flex-1 w-full flex items-center h-12 px-4 bg-slate-50 rounded-xl border border-transparent focus-within:border-[#ff6b6b]/30 transition-all">
                        <span class="material-symbols-outlined text-slate-400 text-[22px]">search</span>
                        <input name="q" value="<?= htmlspecialchars($search_query ?? '') ?>" class="bg-transparent border-0 outline-none text-slate-800 placeholder-slate-400 w-full text-sm focus:ring-0 pl-3" placeholder="Nə axtarırsınız?" type="text"/>
                    </div>

                    <div class="w-full sm:w-auto flex items-center h-12 px-4 bg-slate-50 rounded-xl border border-transparent hover:border-[#ff6b6b]/30 transition-all group relative">
    <span class="material-symbols-outlined text-slate-400 text-[22px] group-hover:text-[#ff6b6b] flex-shrink-0">location_on</span>

    <select name="city_id" class="bg-transparent border-0 outline-none text-slate-700 text-[13px] font-bold focus:ring-0 cursor-pointer appearance-none w-full sm:w-40 pl-2 pr-8 relative z-10 bg-none shadow-none">
        <option value="">Bütün Azərbaycan</option>
        <?php if(isset($cities)): foreach($cities as $city): ?>
            <option value="<?= $city['id'] ?>" <?= (isset($city_filter) && $city_filter == $city['id']) ? 'selected' : '' ?>><?= htmlspecialchars($city['name']) ?></option>
        <?php endforeach; endif; ?>
    </select>

    <span class="material-symbols-outlined text-slate-400 text-[18px] absolute right-3 pointer-events-none group-hover:text-[#ff6b6b] z-0">expand_more</span>
</div>

                    <button type="submit"
                            class="w-full sm:w-auto h-12 px-10 text-white rounded-xl text-sm font-black transition-all shadow-lg active:scale-95 hover:brightness-110"
        ...
        <p class="text-slate-900 font-black text-sm sm:text-xl tracking-tight truncate">
            ...
            <span class="text-[9px] sm:text-[11px] font-bold text-slate-400">
                <?= htmlspecialchars($ad['symbol']) ?><?= ($ad['listing_type'] ?? 'rent') === 'rent' ? ($rent_labels[$ad['rent_period']] ?? '/gün') : '' ?>
                        ...
                        <button class="flex-shrink-0 p-1.5 sm:p-2.5 rounded-lg bg-slate-900 text-white hover:bg-[#ff6b6b] transition-all shadow-md">
