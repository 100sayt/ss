...
if (!empty($search_query)) {
    ...
    // Saitləri MySQL-in başa düşəcəyi "_" (hər hansı tək simvol) ilə əvəzləyirik
...
if ($cat_filter) {
    $normal_sql .= " AND (l.category_id = ? OR c.parent_id = ?)";
...
<div class="flex-1 min-w-0" x-data="{
                ...
                <div class="w-full max-w-2xl relative">
                    <form action="/index.php" method="GET" id="mainSearchForm" class="bg-white p-2 rounded-2xl flex flex-col sm:flex-row items-center gap-2 shadow-2xl">
                        ...
                        <div class="flex items-center gap-2 w-full sm:w-auto">
                            ...
                            <button type="submit" class="flex-1 sm:flex-none h-12 px-8 text-white rounded-xl text-sm font-black transition-all shadow-lg active:scale-95 hover:brightness-110 flex-shrink-0" style="background-color: #ff6b6b;">Axtar</button>
                        </div>
    ...
    <template x-teleport="body">
        <div x-show="filterModalOpen" class="fixed inset-0 z-[99999] flex items-center justify-center p-4" x-cloak>
            ...
            <div x-show="filterModalOpen"
                ...
                <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-white">
                    ...
                    <button @click="filterModalOpen = false" class="w-10 h-10 rounded-xl bg-slate-50 hover:bg-red-50 hover:text-red-500 flex items-center justify-center text-slate-400 transition-all"><span class="material-symbols-outlined">close</span></button>
                </div>

                <div class="flex-1 overflow-y-auto p-6 space-y-6">
                    <!-- Category -->
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Kateqoriya</label>
                        <div class="relative">
                            <select x-model="catId" class="w-full h-12 px-4 bg-slate-50 border-2 border-transparent rounded-xl text-sm font-bold focus:border-[#ff6b6b]/20 focus:bg-white focus:ring-0 outline-none transition-all appearance-none cursor-pointer">
                                <option value="">Bütün kateqoriyalar</option>
                                <?php foreach($categories_data as $cat): ?>
                                    <option value="<?= $cat['id'] ?>"><?= htmlspecialchars($cat['name']) ?></option>
                                <?php endforeach; ?>
                            </select>
                            <span class="material-symbols-outlined text-slate-400 absolute right-4 top-1/2 -translate-y-1/2 pointer-events-none">expand_more</span>
                        </div>
                    </div>

                    <!-- Price -->
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Qiymət (AZN)</label>
                        <div class="flex items-center gap-3">
                            <input type="number" x-model="minPrice" placeholder="Min" class="flex-1 h-12 px-4 bg-slate-50 border-2 border-transparent rounded-xl text-sm font-bold focus:border-[#ff6b6b]/20 focus:bg-white focus:ring-0 outline-none transition-all">
                            <div class="w-3 h-0.5 bg-slate-200"></div>
                            <input type="number" x-model="maxPrice" placeholder="Maks" class="flex-1 h-12 px-4 bg-slate-50 border-2 border-transparent rounded-xl text-sm font-bold focus:border-[#ff6b6b]/20 focus:bg-white focus:ring-0 outline-none transition-all">
                        </div>
                    </div>

                    <!-- Deposit -->
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Depozit</label>
                        <div class="grid grid-cols-3 gap-2">
                            <template x-for="opt in [{v:'',l:'Hamısı'}, {v:'1',l:'Var'}, {v:'0',l:'Yox'}]">
                                <button type="button" @click="hasDeposit = opt.v" :class="hasDeposit == opt.v ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-600 border-transparent'" class="h-10 rounded-xl text-[10px] font-black uppercase tracking-tighter border-2 transition-all" x-text="opt.l"></button>
                            </template>
                        </div>
                    </div>
                    ...
                    <!-- Sorting -->
                    <div>
                        <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Sıralama</label>
                        <div class="grid grid-cols-2 gap-2">
                            <template x-for="opt in [{v:'newest',l:'Yeni'}, {v:'oldest',l:'Köhnə'}, {v:'price_asc',l:'Ucuz'}, {v:'price_desc',l:'Baha'}]">
                                <button type="button" @click="sortOrder = opt.v" :class="sortOrder == opt.v ? 'border-[#ff6b6b] bg-[#ff6b6b]/5 text-[#ff6b6b]' : 'border-slate-100 text-slate-600'" class="h-11 rounded-xl text-[11px] font-bold border-2 transition-all" x-text="opt.l"></button>
                            </template>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 border-t border-slate-100 bg-white flex gap-3">
                    <button type="button" @click="minPrice=''; maxPrice=''; hasDeposit=''; sortOrder='newest'; catId=''; document.getElementById('mainSearchForm').submit()" class="flex-1 h-12 text-xs font-black text-slate-400 uppercase tracking-widest hover:text-red-500 transition-colors">Sıfırla</button>
                    <button type="button" @click="document.getElementById('mainSearchForm').submit()" class="flex-[2] h-12 bg-[#ff6b6b] text-white rounded-xl text-sm font-black uppercase tracking-widest hover:brightness-110 transition-all shadow-lg shadow-[#ff6b6b]/20">Göstər</button>
                </div>
            </div>
        </div>
    </template>
