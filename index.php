...
$has_deposit_filter = isset($_GET['has_deposit']) && $_GET['has_deposit'] !== '' ? (int)$_GET['has_deposit'] : '';
...
$filter_sql_part = "";
$filter_params = [];
if ($min_price !== '') { $filter_sql_part .= " AND l.price >= ?"; $filter_params[] = (float)$min_price; }
if ($max_price !== '') { $filter_sql_part .= " AND l.price <= ?"; $filter_params[] = (float)$max_price; }
if ($has_deposit_filter !== '') { $filter_sql_part .= " AND l.has_deposit = ?"; $filter_params[] = (int)$has_deposit_filter; }
...
$categories_main = $stmt_cats->fetchAll();
...
$premium_listings_stmt = $pdo->prepare($premium_sql);
$premium_listings_stmt->execute($premium_params);
$premium_listings = $premium_listings_stmt->fetchAll();
...
$normal_listings_stmt = $pdo->prepare($normal_sql);
$normal_listings_stmt->execute($normal_params);
$normal_listings = $normal_listings_stmt->fetchAll();

$cities_all = $pdo->query("SELECT id, " . lang_col('name') . " as name FROM cities")->fetchAll();
                    ...
                    <form action="/index.php" method="GET" id="mainSearchForm" class="bg-white p-2 rounded-2xl flex flex-col sm:flex-row items-center gap-2 shadow-2xl">
                        <div class="flex-1 w-full flex items-center h-12 px-4 bg-slate-50 rounded-xl border border-transparent focus-within:border-[#ff6b6b]/30 transition-all relative">
                            ...
                            <div x-show="searchSuggestions.length > 0"
                                ...
                                <template x-for="item in searchSuggestions" :key="item.id">
                                    <a :href="'/listing.php?id=' + item.id" class="search-suggestion-item">
                                        <span class="material-symbols-outlined text-slate-300 text-sm">search</span>
                                        <span class="text-sm font-bold truncate" x-text="item.title"></span>
                                    </a>
                                </template>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center gap-2 w-full sm:w-auto">
                            ...
                            <div class="flex-1 sm:flex-none flex items-center h-12 px-4 bg-slate-50 rounded-xl border border-transparent hover:border-[#ff6b6b]/30 transition-all group relative">
                                <span class="material-symbols-outlined text-slate-400 text-[22px] flex-shrink-0">location_on</span>
                                <select name="city_id" class="bg-transparent border-0 outline-none text-slate-700 text-[13px] font-bold focus:ring-0 cursor-pointer appearance-none w-full sm:w-32 pl-2 pr-6 bg-none">
                                    <option value="">Bütün ölkə</option>
                                    <?php foreach($cities_all as $city): ?>
                                        <option value="<?= $city['id'] ?>" <?= ($city_filter == $city['id']) ? 'selected' : '' ?>><?= htmlspecialchars($city['name']) ?></option>
                                    <?php endforeach; ?>
                                </select>
                                <span class="material-symbols-outlined text-slate-400 text-[18px] absolute right-2 pointer-events-none group-hover:text-[#ff6b6b]">expand_more</span>
                            </div>
                            ...
                            <!-- Search Button -->
                            <button type="submit" class="flex-1 sm:flex-none h-12 px-8 text-white rounded-xl text-sm font-black transition-all shadow-lg hover:brightness-110 flex-shrink-0" style="background-color: #ff6b6b;">Axtar</button>
                        </div>

                        <!-- Hidden inputs to sync with modal -->
        ...
        <?php foreach ($premium_listings as $ad): ?>
            <div class="glass-panel rounded-[1.2rem] sm:rounded-[1.5rem] overflow-hidden glow-effect transition-all duration-500 group flex flex-col h-full bg-white border border-slate-100 hover:-translate-y-1.5 cursor-pointer shadow-sm hover:shadow-xl" onclick="window.location.href='/listing.php?id=<?= (int)$ad['id'] ?>'">
                <div class="relative h-32 sm:h-48 w-full overflow-hidden">
                    ...
                    <button class="absolute top-2 right-2 z-10 p-1.5 sm:p-2 bg-white/90 backdrop-blur-md rounded-full text-slate-400 hover:text-red-500 transition-colors shadow-sm" onclick="event.stopPropagation(); if(typeof toggleFavorite === 'function') toggleFavorite(<?= (int)$ad['id'] ?>, this)"><span class="material-symbols-outlined text-[16px] sm:text-[18px] <?= (function_exists('is_favorite') && is_favorite($pdo, $ad['id'])) ? 'text-red-500' : '' ?>" style="<?= (function_exists('is_favorite') && is_favorite($pdo, $ad['id'])) ? "font-variation-settings: 'FILL' 1" : '' ?>">favorite</span></button>
